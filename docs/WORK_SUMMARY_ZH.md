...
- **没有服务器也能实时同步 Scholar。** GitHub Actions 每 6 小时运行一次，
  通过 `scholarly` 抓取 Scholar 资料页，写入 `data/scholar.json`。客户端每次
  打开页面都会 fetch 这个 JSON，并以动画呈现数字。抓取失败时会保留上次快照
  ，只更新时间戳，保证公开数字不会瞬间归零。
...
- `.github/workflows/update-scholar.yml` — 每 6 小时的 cron + 手动触发
